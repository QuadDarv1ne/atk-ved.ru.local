...
class ATK_VED_Image_Optimizer {
	...
	private function __construct() {
		...
		add_filter( 'wp_get_attachment_image_src', array( $this, 'use_webp_if_exists' ), 10, 4 );
		add_filter( 'wp_get_attachment_image_attributes', array( $this, 'add_lazy_loading' ), 10, 3 );
		add_filter( 'wp_get_attachment_image_attributes', array( $this, 'add_srcset_webp' ), 20, 3 );
		add_filter( 'post_thumbnail_html', array( $this, 'optimize_post_thumbnail' ), 10, 5 );
		add_filter( 'content_img_tag', array( $this, 'optimize_content_images' ), 10, 3 );
		add_action( 'wp_head', array( $this, 'add_preload_hero_images' ), 10 );
		add_filter( 'wp_lazy_loading_enabled', array( $this, 'selective_lazy_loading' ), 10, 3 );
	}
	...
	public function generate_webp_on_upload( array $metadata, int $attachment_id ): array {
		...
		if ( $extension === 'webp' || ! in_array( $extension, array( 'jpg', 'jpeg', 'png', 'gif' ), true ) ) {
			return $metadata;
		}
...
add_action( 'delete_attachment', array( ATK_VED_Image_Optimizer::get_instance(), 'clean_webp_on_delete' ) );
