...
function atk_ved_validate_coupon(string $code, float $cart_total = 0, int $user_id = 0, string $user_email = ''): array {
    ...
    if (!$coupon) {
        return array('valid' => false, 'message' => __('Промокод не найден', 'atk-ved'));
    }
    ...
    if ($coupon->valid_from && $now < $coupon->valid_from) {
        return array('valid' => false, 'message' => __('Промокод ещё не активен', 'atk-ved'));
    }
    
    if ($coupon->valid_until && $now > $coupon->valid_until) {
        return array('valid' => false, 'message' => __('Срок действия промокода истёк', 'atk-ved'));
    }
    
    // Проверка лимита использований
    if ($coupon->usage_limit > 0 && $coupon->usage_count >= $coupon->usage_limit) {
        return array('valid' => false, 'message' => __('Лимит использований промокода исчерпан', 'atk-ved'));
    }
    ...
    if ($user_id > 0 || !empty($user_email)) {
        ...
        if ($coupon->user_usage_limit > 0 && $user_usage >= $coupon->user_usage_limit) {
            return array('valid' => false, 'message' => __('Вы уже использовали этот промокод', 'atk-ved'));
        }
    }
