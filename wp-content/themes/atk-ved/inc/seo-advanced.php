...
class ATK_VED_SEO {
	...
	private function __construct() {
		// Schema.org
		add_action( 'wp_head', array( $this, 'output_schema_org' ), 1 );
		add_action( 'wp_footer', array( $this, 'output_schema_jsonld' ), 1 );
	...
	private function get_organization_schema(): array {
		...
		$schema['contactPoint'] = array(
			array(
				...
				'availableLanguage' => array( 'Russian', 'Chinese' ),
				'areaServed'        => 'RU',
			),
		);
	...
	private function get_faq_schema(): ?array {
		$faqs = get_field( 'faq_items', 'option' ) ?? array();
	...
	private function get_review_schema(): ?array {
		$reviews = get_field( 'reviews', 'option' ) ?? array();
