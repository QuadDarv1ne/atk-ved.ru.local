...
function atk_ved_create_loyalty_tables(): void {
    ...
    if ($existing_tiers == 0) {
        $wpdb->insert($tiers_table, array('name' => 'Бронзовый', 'slug' => 'bronze', 'min_points' => 0, 'discount_percent' => 0, 'benefits' => 'Базовый уровень'));
        $wpdb->insert($tiers_table, array('name' => 'Серебряный', 'slug' => 'silver', 'min_points' => 1000, 'discount_percent' => 3, 'benefits' => 'Скидка 3% на все товары'));
        $wpdb->insert($tiers_table, array('name' => 'Золотой', 'slug' => 'gold', 'min_points' => 5000, 'discount_percent' => 5, 'benefits' => 'Скидка 5% + бесплатная доставка'));
        $wpdb->insert($tiers_table, array('name' => 'Платиновый', 'slug' => 'platinum', 'min_points' => 10000, 'discount_percent' => 10, 'benefits' => 'Скидка 10% + приоритетная поддержка'));
    }
}
...
function atk_ved_add_loyalty_points(int $user_id, int $points, string $type, string $description = '', string $ref_type = '', int $ref_id = 0): void {
    ...
    // Обновляем баланс
    $wpdb->update($balance_table, array(
        'balance' => $new_balance,
...
Ваш уровень лояльности повышен с %s до %s!

Теперь вам доступна персональная скидка на все товары.

Спасибо, что вы с нами!', 'atk-ved'),
        $user->display_name,
        ...
        <div class="loyalty-progress">
            ...
            <p>
                ...
                if ($balance['points_to_next_tier'] > 0) {
                    printf(
                        __('Ещё <strong>%d баллов</strong> до уровня %s', 'atk-ved'),
