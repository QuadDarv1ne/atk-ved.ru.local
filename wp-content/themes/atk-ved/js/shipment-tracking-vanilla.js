/**
 * JavaScript для отслеживания грузов (Vanilla JS)
 * 
 * @package ATK_VED
 * @since 2.0.0
 */
...
(function() {
    ...
    function init() {
        ...
        const resultContent = result?.querySelector('.tracking-result-content');
        const error = $('#trackingError');
        const loader = result?.querySelector('.tracking-loader');
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showError(data.data?.message || 'Груз не найден');
                }
            })
            .catch(() => {
                if (loader) loader.style.display = 'none';
        ...
        function displayTrackingInfo(data) {
            ...
            // Analytics
            if (typeof ym !== 'undefined' && window.atkVed?.metrikaId) {
                ym(window.atkVed.metrikaId, 'reachGoal', 'tracking_used');
            }
